...
- **Testing error paths under ESM:** you cannot monkey-patch `node:*` namespace members (e.g. `(fs as any).readdirSync = …`) — ESM namespace objects are sealed/read-only in Node 22+. To make a built-in I/O call substitutable for a test (e.g. to simulate `EACCES`), export an `@internal` mutable hooks object from the module and have the implementation call through it; tests reassign the hook and restore it in a `finally`. See `src/walkFiles.ts` (`_walkFilesHooks`) for the established pattern. Keep the hooks object out of `src/index.ts` so it stays off the public API.
