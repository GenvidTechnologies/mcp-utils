...
**Below 1.0.0 a caret pin excludes every minor bump — which is what makes the rule above protective.** `^0.8.0` resolves as `>=0.8.0 <0.9.0`: while the major is `0`, npm's caret permits **patch** updates only. So the two choices differ by far more than one digit. A patch reaches every caret-pinned consumer on their next install, unannounced — that is the "inherit the change silently" case above, and precisely what the minor-bump rule exists to prevent. A minor reaches nobody until they widen the range themselves, which is why downstream adoption here is always deliberate work and never a transparent pickup. **State which of the two a release is in its CHANGELOG entry, and check the direction before you write it.** The `0.8.0` release commit gets it right for `^0.7.0` consumers; a 2026-08-28 draft of the `0.9.0` entry asserted the exact inverse ("lands within their existing range") and lint, typecheck, 250 tests and a clean build were all green over it — nothing here reads prose. See [instance 9](wiki/failure-modes-that-report-success.md).
