...
`wiki/` is not built, linted, typechecked, or published (it ships nowhere near `dist/`), so `npm run validate` will never catch an error in it. **One mechanical check does reach it, as of #17:** the `docs/TOC.md` → `wiki/index.md` override sets `audit-conventions`' `docsRoot` to `wiki`, and its three hygiene scanners — broken intra-repo links, orphaned pages, retired tokens — walk `<docsRoot>/**.md` + `CLAUDE.md`. That is how five broken links in `wiki/process/code-review-context.md` were caught during #17. It is not run by CI, so it only fires when someone invokes `/gvt-dev:audit-conventions`. Everything else about the wiki is still unchecked — see `wiki/failure-modes-that-report-success.md`, which is itself about that class of gap.
