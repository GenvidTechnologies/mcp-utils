...
`wiki/` is not built, linted, typechecked, or published (it ships nowhere near `dist/`), so `npm run validate` will never catch an error in it. **`audit-conventions`' hygiene scanners do reach it, and #17 widened that reach.** Their scope is per-scanner, not one shared walk (see `CONVENTIONS.md`): all three walk the docs root plus repo-root `CLAUDE.md`, and `scanRetiredTokens` *additionally* walks `<wikiDir>/`. So retired-token scanning of `wiki/` predates #17; what #17 changed is the docs root itself — the `docs/TOC.md` → `wiki/index.md` override makes it `wiki`, which is what brought **broken-link and orphaned-page** checking to these pages for the first time. That is how five broken links in `wiki/process/code-review-context.md` were caught. None of it runs in CI, so it only fires when someone invokes `/gvt-dev:audit-conventions`. Everything else about the wiki is still unchecked — see `wiki/failure-modes-that-report-success.md`, which is itself about that class of gap.

**Write ordinary relative wiki links (`../index.md`), not the bundle-absolute form (`/index.md`)** — even though `wiki/wiki-schema.md` §6.1 *recommends* bundle-absolute as the preferred form. `scanBrokenLinks` resolves links against the filesystem, so it reads a bundle-absolute link as filesystem-absolute and reports every one broken; `burbank-playfab` measured **312 false positives** after making its docs root an OKF bundle, exactly the configuration #17 created here. We are unexposed only because no page currently uses that form. Tracked as [gvt-dev#421](https://github.com/GenvidTechnologies/claude-code-plugin-gvt-dev/issues/421) — **delete this paragraph when it lands.**
...
- **Testing an MCP surface: drive a real client over `InMemoryTransport`.** A helper that registers resources or tools (`exposeDocs`) cannot be meaningfully tested by calling it and asserting it doesn't throw — that exercises registration only. Build an `McpServer`, register against it, and connect a `Client` from `@modelcontextprotocol/sdk/client/index.js` through `InMemoryTransport.createLinkedPair()`; then `listResources()` / `readResource()` exercise the real request path. This matters for error assertions specifically: a handler's thrown `McpError` is converted to a JSON-RPC error **by the request machinery**, so the conversion is simply not observable if you invoke the callback directly. Note `InMemoryTransport` is reached through the package's `./*` wildcard export (`@modelcontextprotocol/sdk/inMemory.js`), not a named entry in the `exports` map.
- **No `chai-as-promised`.** Async rejection assertions (`.to.be.rejectedWith(...)`) are unavailable — `chai` is the only assertion dependency, and adding one for this would sit badly with the dependency-light constraint. Await the promise in a `try`/`catch` and assert on the returned message instead; `test/exposeDocs.test.ts`'s `rejectionMessage` helper is the established shape. Assert what the message *does not* contain too (a raw `ENOENT`, an absolute host path) — that is usually the actual contract.
...
- `txToken.ts` — `isValidProjectId`, `formatTxToken`, `parseTxToken`, `compareTxToken`: mint and parse the wire representation of `OptimisticWatcher`'s `txId` — a plain-string `TxToken` shaped `` `${projectId}:${n}` ``. This is a **wire contract**, not an incidental string format: it has two named consumers, `GenvidTechnologies/c3-domain-manager#77` and `GenvidTechnologies/construct3-chef#95`, so the delimiter and the number shape are fixed rather than free to drift. `formatTxToken` is the one function that throws (`TypeError`) — the deliberate exception to this module's contract, since its input comes from the server's own construction path rather than off the wire. `parseTxToken` and `compareTxToken` are total: they never throw, returning `null` / `false` respectively for any malformed input, including non-string input. The `n` half is **strictly canonical** — leading zeros, signs, whitespace, exponent notation, and hex are all rejected, and the value must additionally be a safe integer, capped at `Number.MAX_SAFE_INTEGER`, so a shape-valid but overlarge digit string is rejected rather than coerced lossily. Rationale and rejected alternatives: [ADR-0005](wiki/decisions/0005-tx-token-wire-format.md).

**Filesystem & path**

- `walkFiles.ts` — `walkFiles(dir, match, readdir?, stat?)`: recursively returns absolute paths of files matching a suffix string or predicate. **Guarantees every returned path is a regular file** — directories, symlinks to directories (incl. Windows junctions), broken symlinks and cycles are excluded even when their *name* matches; symlinks to regular files are kept, since reading them succeeds. Missing dir → `[]`; other `readdir` errors re-thrown; symlinked dirs not followed (which is also what bounds the walk). Classification is dirent-first, falling back to a resolving `stat` only for symlinks/special entries that already matched. A failed `stat` **drops the entry** rather than propagating — deliberately asymmetric with the `readdir` policy, because `throwIfNoEntry: false` suppresses only `ENOENT` (a cycle throws `ELOOP`), and failing to classify one leaf shouldn't abort a walk. Rationale and rejected alternatives: [ADR-0001](wiki/decisions/0001-walkfiles-returns-only-regular-files.md). `readdir` and `stat` are injectable test seams (see ESM error-path note above).
...
- `exposeDocs.ts` — `exposeDocs(server, packageDir, opts?)`: registers MCP resources serving a documentation directory's `*.md` (templated `docs:///{+path}`) and `README.md` (`docs:///readme`) from a consuming package's directory. `opts.docsDir` (default `"docs"`) names the directory relative to `packageDir`; `opts.recursive` (default `false`) descends subdirectories, exposing **path-shaped, extension-less** names (`wiki/reference/cli.md` → `docs:///reference/cli`). Reserved expansion (`{+path}`) also matches a separator-free name, so every flat URI the old `{name}` template served is unchanged. `recursive` gates **serving**, not just listing — a nested name is refused when it's off. The template now supplies a real `list` callback, so the doc set is enumerable via `resources/list` (it previously contributed nothing). A `<docsDir>/readme.md` is omitted from listings/completions when `README.md` exists, since the static resource owns `docs:///readme` and the SDK resolves an exact resource before any template (it was never readable; it only became *visible* once the template started enumerating). Scans through `walkFiles`, so only regular files are offered; names are built with `toPosixPath`; reads are guarded by `resolveWithin` and an unresolvable name raises `McpError(InvalidParams)` rather than a raw `ENOENT`. That guard is **defence in depth, not a reachable-escape fix** — the SDK normalises the URI through `new URL()` before matching, which already collapses `..`. Rationale, the end-to-end measurement, and how the first version of the record got this wrong: [ADR-0003](wiki/decisions/0003-exposedocs-path-shaped-resource-names.md).
...
- `package.json` `files` is an **allow-list** (`["dist","LICENSE","README.md"]`), and `wiki/` is not in it — nor was `docs/` before it was retired. Anything moved out of `README.md` **leaves the npm tarball** — package-page readers would get links to GitHub instead of the API docs.
...
Revisit when the grouped index stops fitting on one screen (roughly 20–24 utilities). The likely shape is README keeping install + quickstart + the index with one-line blurbs, and per-utility docs moving to `wiki/api/<name>.md` — but because of the tarball cost above, that move warrants an ADR rather than a judgment call in a PR.
